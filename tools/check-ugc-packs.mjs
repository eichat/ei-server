...
// ── Відхилений набір не має лишатись у публічному бакеті ──────────────────
// Файли кладе туди подача, ще ДО розгляду, тож без прибирання вміст, який
// модерація визнала неприйнятним, лишався б доступним за прямим посиланням.
// Перевірка статична: виклик легко зникає при рефакторі, і помітити це на
// живому сервері можна лише зазирнувши в бакет.
{
  const i = src.indexOf("app.post('/admin/sticker-review'");
  const body = i < 0 ? '' : src.slice(i, src.indexOf('\n});', i));
  eq('review існує', i >= 0, true);
  eq('відхилення прибирає файли з бакета', /rejected[\s\S]{0,600}ugcRemovePackFiles\(/.test(body), true);
  eq('відхилення прибирає й позиції набору', /rejected[\s\S]{0,700}sticker_pack_items'\)\.delete\(/.test(body), true);
  // unpublish навпаки: набір лишається схваленим, покупці мають ним користуватись
  eq('unpublish файлів не чіпає', /unpublish[^\n]*\n[^\n]*ugcRemovePackFiles/.test(body), false);
}
