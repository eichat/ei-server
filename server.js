  ...
  ws.on('message', async (raw) => {
    try {
      ...
      if (msg.type === 'login') {
        ...
        if (pendingReactions && pendingReactions.length > 0) { console.log(`[reaction] pending дозасил to=${userNick} n=${pendingReactions.length} groups=${pendingReactions.map(r => r.group_id ?? 'direct').join(',')}`); for (const r of pendingReactions) ws.send(JSON.stringify({ type: 'reaction', msgId: r.msg_id, emoji: r.emoji, from: r.from_nick, chatNick: r.chat_nick, groupId: r.group_id != null ? Number(r.group_id) : null })); await supabase.from('pending_reactions').delete().eq('to_nick', userNick); }
      ...
      if (msg.type === 'reaction') { const { msgId, emoji, chatNick, groupId } = msg; const payload = { type: 'reaction', msgId, emoji, from: userNick, chatNick, groupId }; if (groupId) { const { data: ex } = await supabase.from('group_message_reactions').select('id').eq('msg_id', msgId).eq('group_id', groupId).eq('nick', userNick).eq('emoji', emoji).maybeSingle(); if (ex) { await supabase.from('group_message_reactions').delete().eq('id', ex.id); } else { await supabase.from('group_message_reactions').delete().eq('msg_id', msgId).eq('group_id', groupId).eq('nick', userNick); await supabase.from('group_message_reactions').insert({ msg_id: msgId, group_id: groupId, nick: userNick, emoji }); } const { data: members, error: memErr } = await supabase.from('group_members').select('nick').eq('group_id', groupId); const rcpt = (members || []).filter(m => m.nick !== userNick); const dbg = []; for (const m of rcpt) { const live = sendToUser(m.nick, payload); dbg.push(`${m.nick}:${live ? 'live' : (onlineUsers.has(m.nick) ? 'stale' : 'offline')}`); if (!live) { const { error: pErr } = await supabase.from('pending_reactions').insert({ msg_id: msgId, emoji, from_nick: userNick, to_nick: m.nick, group_id: groupId, chat_nick: null }); if (pErr) console.log(`[reaction] pending INSERT FAILED to=${m.nick}: ${pErr.message}`); } } console.log(`[reaction] group=${groupId} msg=${msgId} from=${userNick} members=${rcpt.length}${memErr ? ' memErr=' + memErr.message : ''} -> ${dbg.join(', ') || 'НІКОМУ'}`); } else if (chatNick) { const pairKey = [userNick, chatNick].sort().join('|'); const { data: dex } = await supabase.from('direct_message_reactions').select('id').eq('msg_id', msgId).eq('from_nick', userNick).eq('emoji', emoji).maybeSingle(); if (dex) { await supabase.from('direct_message_reactions').delete().eq('id', dex.id); } else { await supabase.from('direct_message_reactions').delete().eq('msg_id', msgId).eq('from_nick', userNick).eq('pair_key', pairKey); await supabase.from('direct_message_reactions').insert({ msg_id: msgId, from_nick: userNick, emoji, pair_key: pairKey }); } const live = sendToUser(chatNick, payload); console.log(`[reaction] direct to=${chatNick} msg=${msgId} from=${userNick} -> ${live ? 'live' : (onlineUsers.has(chatNick) ? 'stale' : 'offline')}`); if (!live) { const { error: pErr } = await supabase.from('pending_reactions').insert({ msg_id: msgId, emoji, from_nick: userNick, to_nick: chatNick, chat_nick: chatNick, group_id: null }); if (pErr) console.log(`[reaction] pending INSERT FAILED to=${chatNick}: ${pErr.message}`); } } }
