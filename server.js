        ...
        if (myGroups && myGroups.length > 0) {
          for (const gm of myGroups) {
            ...
            if (pendingGroup && pendingGroup.length > 0) { for (const m of pendingGroup) {
              if (m.type === 'file') {
                ws.send(JSON.stringify({ type: 'file_message', groupId: m.group_id, from: m.from_nick, fileName: m.file_name, data: m.file_data, timestamp: m.timestamp, msgId: m.msg_id }));
              } else {
